...
func (gw *Gateway) handleReceive(msg config.Message) {
	gw.logger.Debugf("Received message: %#v", msg)
}
...
func (gw *Gateway) SendMessage(
	...
	if extra, ok := msg.Extra["slack_attachment"]; ok {
		...
		gw.logger.Infof("Thread ID: %s, fromURL: %s, Channel ID: %s, Channel Name: %s, User ID: %s, Username: %s, Message ID: %s, Parent ID: %s, Text: %s Message Object: %#v, Channel Object: %#v", msg.ThreadID, fromURL, getChannelID(channel.ID), getChannelID(msg.Channel), msg.UserID, msg.Username, msg.ID, msg.ParentID, msg.Text, msg, channel)
		msg.Text = msg.Text + "\n> *In reply to: " + "https://discord.com/channels/" + dest.GetString("Server") + "/" + getChannelID(rmsg.Channel) + "/" + strings.Replace(msg.ThreadID, dest.Protocol+" ", "", 1) + "*"
	} else {
